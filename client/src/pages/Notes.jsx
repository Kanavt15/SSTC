...
import { FiSearch, FiFilter, FiUpload, FiDownload, FiStar, FiFileText, FiX, FiFile } from 'react-icons/fi';
import './Notes.css';
...
const Notes = () => {
  ...
  const fetchNotes = async () => {
    ...
    try {
      ...
      console.error(err);
      // Demo data when server isn't connected
      setNotes([
        { id: 1, title: 'Data Structures Complete Notes', subject: 'DSA', department: 'COMPS', year: 'SE', semester: 3, note_type: 'notes', download_count: 156, rating_sum: 45, rating_count: 10, first_name: 'Ankit', last_name: 'Sharma', created_at: new Date().toISOString() },
        { id: 2, title: 'Machine Learning PYQ 2025', subject: 'ML', department: 'AIDS', year: 'TE', semester: 5, note_type: 'pyq', download_count: 89, rating_sum: 37, rating_count: 9, first_name: 'Priya', last_name: 'Patel', created_at: new Date().toISOString() },
        { id: 3, title: 'Computer Networks Lab Manual', subject: 'CN', department: 'IT', year: 'TE', semester: 5, note_type: 'notes', download_count: 234, rating_sum: 52, rating_count: 12, first_name: 'Rahul', last_name: 'Desai', created_at: new Date().toISOString() },
        { id: 4, title: 'DBMS Important Questions', subject: 'DBMS', department: 'COMPS', year: 'SE', semester: 4, note_type: 'pyq', download_count: 312, rating_sum: 68, rating_count: 15, first_name: 'Sneha', last_name: 'Kulkarni', created_at: new Date().toISOString() },
        { id: 5, title: 'Signals & Systems Notes', subject: 'SS', department: 'EXTC', year: 'SE', semester: 3, note_type: 'notes', download_count: 98, rating_sum: 25, rating_count: 6, first_name: 'Vikram', last_name: 'Joshi', created_at: new Date().toISOString() },
        { id: 6, title: 'OOP Concepts Simplified', subject: 'OOP', department: 'COMPS', year: 'SE', semester: 3, note_type: 'notes', download_count: 178, rating_sum: 40, rating_count: 9, first_name: 'Neha', last_name: 'More', created_at: new Date().toISOString() },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  const getRating = (note) => note.rating_count > 0 ? (note.rating_sum / note.rating_count).toFixed(1) : 'N/A';
        ...
        <div className="notes-grid stagger">
          {loading ? (
            Array(6).fill(0).map((_, i) => (
              <div key={i} className="note-card-skeleton skeleton" style={{ height: 220 }} />
            ))
          ) : notes.length > 0 ? (
            notes.map((note) => (
              <div key={note.id} className="note-card card animate-fade-in" id={`note-${note.id}`}>
                <div className="note-card-header">
                  ...
                  <div className="note-card-rating">
                    <FiStar className="star-icon" /> {getRating(note)}
                  </div>
                </div>
                ...
                <div className="note-card-footer">
                  ...
                  <div className="note-card-downloads">
